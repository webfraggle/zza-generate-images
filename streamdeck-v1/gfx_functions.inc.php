...
function addResizedTextToImage($text,$size,$font,$color,$xFactor,$yFactor,$im,$x,$y,$align="left",$paint=true,$linespacing=1)
{
    ...
    $bbox = imageftbbox($size*$fontfactor*$upscale, 0, $font, $text,array("linespacing" => $linespacing));
    ...
    imagefttext($textImage, $size*$fontfactor*$upscale, 0, $bbox[0], $height-$bbox[1]+$offset, $fontColor, $font, $text,array("linespacing" => $linespacing));
    ...
    $targetWidth = ceil($width*$downscale*$xFactor);
    $targetHeight = ceil(($height+$offset)*$downscale*$yFactor);
    if ($paint)
    {
        switch ($align) {
            case 'right':
                imagecopyresampled($im, $textImage, $x-$targetWidth, $y-ceil(($height-$bbox[1])*$downscale), 0, 0, $targetWidth, ceil(($height+$offset)*$downscale*$yFactor), $width, $height+$offset);
                break;
            case 'center':
                imagecopyresampled($im, $textImage, round($x-($targetWidth*$downscale)), $y-ceil(($height-$bbox[1])*$downscale), 0, 0, $targetWidth, ceil(($height+$offset)*$downscale*$yFactor), $width, $height+$offset);
                break;
